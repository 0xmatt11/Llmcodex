...
5. Set `X_SELF_USER_ID` to the X display name or handle as it appears in the DM message list. This is required so the bridge can avoid echoing messages that it sent itself.
...
| `X_SELF_USER_ID` | Selenium only | Required X display name/handle as rendered in the DM, used to avoid self-echoes. |
