...
function seleniumEventId(event) {
  if (event.href) return event.href;
  const timestampMs = Date.parse(event.created_at) || 0;
  const attachmentUrls = (event.attachments ?? []).map((attachment) => attachment.url).join(',');
  return `selenium:${timestampMs}:${stableId([event.senderName, event.text, attachmentUrls])}`;
}

function seleniumEventTimestamp(event) {
  if (event.created_at) return Date.parse(event.created_at) || 0;
  const match = String(event.id ?? '').match(/^selenium:(\d+):/);
  ...
  async listDmEvents(_conversationId, { sinceId, maxResults = 50 } = {}) {
    ...
    const events = rawEvents
      ...
      .map((event) => {
        ...
        normalized.id = seleniumEventId({ ...event, ...normalized });
    ...
    if (sinceId && sinceIndex < 0) {
      const sinceTimestamp = seleniumEventTimestamp({ id: sinceId });
      if (sinceTimestamp > 0) {
        return events
          .filter((event) => seleniumEventTimestamp(event) > sinceTimestamp)
          .slice(-maxResults)
          .reverse();
      }
      this.logger.warn({ sinceId }, 'Selenium cursor was not found in visible X messages; skipping visible backlog to avoid duplicates');
      return [];
    }
